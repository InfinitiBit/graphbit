...
class GraphBitBenchmark(BaseBenchmark):
    ...
    async def run_simple_task(self) -> BenchmarkMetrics:
        """Run simple task using DIRECT API call, excluding LLM API time."""
    ...
    async def run_sequential_pipeline(self) -> BenchmarkMetrics:
        """Run sequential pipeline using DIRECT API calls (no workflow overhead), excluding LLM API time."""
        if self.llm_client is None:
            raise RuntimeError("LLM client not initialized. Call setup() first.")
    ...
    async def run_parallel_pipeline(self) -> BenchmarkMetrics:
        """Run parallel pipeline using DIRECT batch API calls (no workflow overhead), excluding LLM API time."""
        if self.llm_client is None:
            raise RuntimeError("LLM client not initialized. Call setup() first.")
    ...
    async def run_complex_workflow(self) -> BenchmarkMetrics:
        """Run complex workflow using direct API calls to avoid workflow system issues, excluding LLM API time."""
        if self.llm_client is None:
            raise RuntimeError("LLM client not initialized. Call setup() first.")
    ...
    async def run_memory_intensive(self) -> BenchmarkMetrics:
        """Run memory-intensive test using direct API call, excluding LLM API time."""
        if self.llm_client is None:
            raise RuntimeError("LLM client not initialized. Call setup() first.")
    ...
    async def run_concurrent_tasks(self) -> BenchmarkMetrics:
        """Run concurrent tasks using batch processing or asyncio.gather, excluding LLM API time."""
        if self.llm_client is None:
            raise RuntimeError("LLM client not initialized. Call setup() first.")

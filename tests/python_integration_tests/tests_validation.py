...
from graphbit import EmbeddingClient, EmbeddingConfig, Executor, LlmClient, LlmConfig, Node, Workflow, configure_runtime, get_system_info, health_check, init
...
class TestAPIKeyValidation:
    ...
    def test_openai_api_key_validation(self) -> None:
        ...
        with pytest.raises((ValueError, RuntimeError)) as exc_info:
            LlmConfig.openai("sk-short")
        assert "short" in str(exc_info.value).lower()

        # Test invalid format
        with pytest.raises((ValueError, RuntimeError)) as exc_info:
            LlmConfig.openai("invalid-format-key")

        # Test very short key
        with pytest.raises((ValueError, RuntimeError)):
            LlmConfig.openai("sk-123")
    ...
    def test_anthropic_api_key_validation(self) -> None:
        ...
        # Test too short API key
        with pytest.raises((ValueError, RuntimeError)) as exc_info:
            LlmConfig.anthropic("sk-ant-short")
        assert "short" in str(exc_info.value).lower()

        # Test invalid format
        with pytest.raises((ValueError, RuntimeError)):
            LlmConfig.anthropic("invalid")
    ...
    def test_huggingface_api_key_validation(self) -> None:
        ...
        with pytest.raises((ValueError, RuntimeError)) as exc_info:
            LlmConfig.huggingface("", "gpt2")
        assert "empty" in str(exc_info.value).lower()

        # Test too short API key for LLM
        with pytest.raises((ValueError, RuntimeError)) as exc_info:
            LlmConfig.huggingface("hf_short", "gpt2")
        assert "short" in str(exc_info.value).lower()

        # Test empty API key for embeddings
        with pytest.raises((ValueError, RuntimeError)) as exc_info:
            EmbeddingConfig.huggingface("", "facebook/bart-base")
        assert "empty" in str(exc_info.value).lower()

        # Test too short API key for embeddings
        with pytest.raises((ValueError, RuntimeError)) as exc_info:
            EmbeddingConfig.huggingface("hf_short", "facebook/bart-base")
        assert "short" in str(exc_info.value).lower()
    ...
    def test_openai_embedding_api_key_validation(self) -> None:
        ...
        # Test too short API key
        with pytest.raises((ValueError, RuntimeError)) as exc_info:
            EmbeddingConfig.openai("sk-short")
        assert "short" in str(exc_info.value).lower()
    ...
    def test_valid_api_key_formats(self) -> None:
        ...
        try:
            # OpenAI format
            config1 = LlmConfig.openai("sk-" + "x" * 48, "gpt-3.5-turbo")
            assert config1.provider() == "openai"

            # Anthropic format
            config2 = LlmConfig.anthropic("sk-ant-" + "x" * 40, "claude-3-sonnet-20240229")
            assert config2.provider() == "anthropic"

            # HuggingFace format
            config3 = LlmConfig.huggingface("hf_" + "x" * 30, "gpt2")
...
class TestWorkflowValidation:
    ...
    def test_single_node_validation(self) -> None:
        ...
        node_types = [Node.agent("test_agent", "Process data", "agent_001"), Node.transform("test_transform", "uppercase"), Node.condition("test_condition", "value > 0")]

        for node in node_types:
            test_workflow = Workflow(f"single_{node.name()}")
    ...
    def test_invalid_connection_validation(self) -> None:
        ...
        workflow = Workflow("invalid_connection")

        agent = Node.agent("agent", "Test agent", "agent_001")
    ...
    def test_self_connection_validation(self) -> None:
        ...
        workflow = Workflow("self_connection")

        agent = Node.agent("self_agent", "Self connecting agent", "agent_001")
    ...
    def test_orphaned_nodes_validation(self) -> None:
        ...
        workflow = Workflow("orphaned_nodes")
        ...
        orphan = Node.agent("orphan", "Orphaned agent", "orphan_001")
...
class TestParameterValidation:
    ...
    def test_executor_parameter_validation(self) -> None:
        ...
        with pytest.raises((ValueError, RuntimeError)):
            Executor(config, timeout_seconds=0)

        with pytest.raises((ValueError, RuntimeError)):
            Executor(config, timeout_seconds=4000)  # Too high

        # Test invalid specialized executor parameters
        with pytest.raises((ValueError, RuntimeError)):
            Executor.new_low_latency(config, timeout_seconds=0)

        with pytest.raises((ValueError, RuntimeError)):
            Executor.new_low_latency(config, timeout_seconds=400)  # Too high for low latency
...
class TestNodeValidation:
    ...
    def test_agent_node_validation(self) -> None:
        ...
        # Test empty name
        with pytest.raises((ValueError, RuntimeError)):
            Node.agent("", "Valid prompt", "agent_001")

        # Test empty prompt
        with pytest.raises((ValueError, RuntimeError)):
            Node.agent("valid_name", "", "agent_001")

        # Test invalid agent ID format
        with contextlib.suppress(ValueError, RuntimeError):
            Node.agent("test", "test prompt", "invalid id with spaces")
...
class TestSystemValidation:
    ...
    def test_cross_component_validation(self) -> None:
        ...
        invalid_workflow = Workflow("invalid")

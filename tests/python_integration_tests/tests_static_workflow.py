...
class TestStaticWorkflowCreation:
    ...
    def test_workflow_validation(self) -> None:
        ...
        workflow = Workflow("validation_test")
...
class TestWorkflowNodeCreation:
    ...
    def test_agent_node_creation(self) -> None:
        ...
        agent_node = Node.agent("agent1", "Process the input data", "agent_001")
    ...
    def test_transform_node_creation(self) -> None:
        ...
        transform_node = Node.transform("transform1", "uppercase")
    ...
    def test_condition_node_creation(self) -> None:
        ...
        condition_node = Node.condition("condition1", "quality_score > 0.5")
...
class TestWorkflowComposition:
    ...
    def sample_workflow(self) -> Any:
        ...
        agent1 = Node.agent("agent1", "Start processing", "agent_001")
        condition1 = Node.condition("condition1", "quality_score > 0.5")
        transform1 = Node.transform("transform1", "uppercase")
    ...
    def test_node_addition(self, sample_workflow: Any) -> None:
        ...
        # Add another node
        new_agent = Node.agent("agent2", "Continue processing", "agent_002")
    ...
    def test_workflow_connection_creation(self, sample_workflow: Any) -> None:
        ...
        workflow = Workflow("connection_test")
...
class TestWorkflowExecution:
    ...
    def executable_workflow(self) -> Any:
        ...
        workflow = Workflow("executable_test")

        # Create a simple agent node
        agent_node = Node.agent("test_agent", "Say hello", "agent_001")
...
class TestWorkflowValidation:
    ...
    def test_single_node_validation(self) -> None:
        ...
        workflow = Workflow("single_node_test")

        agent_node = Node.agent("solo_agent", "Work alone", "agent_001")
        workflow.add_node(agent_node)
    ...
    def test_connected_nodes_validation(self) -> None:
        ...
        workflow = Workflow("connected_test")

        # Create two nodes
        agent1 = Node.agent("agent1", "First step", "agent_001")
        agent2 = Node.agent("agent2", "Second step", "agent_002")
...
class TestWorkflowComponents:
    ...
    def test_agent_node_creation(self) -> None:
        ...
        # Basic agent node
        agent1 = Node.agent("basic_agent", "Basic prompt", "agent_001")
        assert agent1.name() == "basic_agent"

        # Agent with auto-generated ID
        agent2 = Node.agent("auto_agent", "Auto ID prompt")
        assert agent2.name() == "auto_agent"

        # Complex agent
        agent3 = Node.agent("complex_agent", "Complex multi-line prompt\nwith instructions", "complex_001")
    ...
    def test_condition_node_variations(self) -> None:
        ...
        cond1 = Node.condition("simple_check", "score > 0.5")
        assert cond1.name() == "simple_check"

        # Complex condition
        cond2 = Node.condition("complex_check", "sentiment == 'positive' and confidence > 0.8")
    ...
    def test_transformation_node_variations(self) -> None:
        ...
        trans1 = Node.transform("uppercase", "uppercase")
        assert trans1.name() == "uppercase"

        # Complex transformation
        trans2 = Node.transform("json_parse", "parse_json")
...
class TestWorkflowMetadata:
    ...
    def test_workflow_properties(self) -> None:
        ...
        workflow = Workflow("metadata_test")
    ...
    def test_workflow_node_metadata(self) -> None:
        ...
        agent_node = Node.agent("meta_agent", "Test metadata", "meta_001")
    ...
    def test_workflow_modification(self) -> None:
        ...
        workflow = Workflow("modify_test")

        # Add nodes
        node1 = Node.agent("node1", "First node", "agent_001")
        node2 = Node.agent("node2", "Second node", "agent_002")
    ...
    def test_workflow_node_retrieval(self) -> None:
        ...
        workflow = Workflow("retrieval_test")

        # Add a node
        agent_node = Node.agent("retrievable", "Test retrieval", "retrieve_001")
    ...
    def test_nested_conditional_workflows(self) -> None:
        ...
        workflow = Workflow("nested_conditional")
        ...
        priority_check = Node.condition("priority_check", "priority == 'high'")
        type_check = Node.condition("type_check", "type == 'urgent'")
        workflow.add_node(priority_check)
        workflow.add_node(type_check)

        # Processing nodes for different paths
        fast_track = Node.agent("fast_track", "Fast track processing", "fast_001")
        standard_process = Node.agent("standard_process", "Standard processing", "std_001")
    ...
    def test_workflow_with_multiple_transforms(self) -> None:
        ...
        workflow = Workflow("multi_transform")

        # Create transformation pipeline
        input_node = Node.agent("input", "Process input", "input_001")
        normalize = Node.transform("normalize", "normalize_data")
        validate_data = Node.transform("validate", "validate_format")
        enrich = Node.transform("enrich", "enrich_metadata")
        output_node = Node.agent("output", "Process output", "output_001")
...
class TestWorkflowErrorHandling:
    ...
    def test_circular_dependency_detection(self) -> None:
        ...
        workflow = Workflow("circular_test")

        node1 = Node.agent("node1", "First node", "circ_001")
        node2 = Node.agent("node2", "Second node", "circ_002")
        node3 = Node.agent("node3", "Third node", "circ_003")
    ...
    def test_invalid_connection_attempts(self) -> None:
        ...
        workflow = Workflow("invalid_connections")

        node1 = Node.agent("node1", "Valid node", "valid_001")
        id1 = workflow.add_node(node1)
...
class TestWorkflowContextManagement:
    ...
    def test_workflow_result_detailed_access(self, llm_config: Any) -> None:
        ...
        agent = Node.agent("context_agent", "Process with context", "ctx_001")
...
class TestWorkflowNodeManipulation:
    ...
    def test_node_id_consistency(self) -> None:
        ...
        workflow = Workflow("node_id_test")
        ...
        for i in range(5):
            node = Node.agent(f"agent_{i}", f"Agent {i}", f"agent_{i:03d}")
            nodes.append(node)
    ...
    def test_different_node_types_integration(self) -> None:
        ...
        workflow = Workflow("mixed_node_types")

        # Create different types of nodes
        agent_node = Node.agent("agent", "Process data", "agent_001")
        condition_node = Node.condition("condition", "data_valid == true")
        transform_node = Node.transform("transform", "normalize_data")
...
class TestComplexWorkflowScenarios:
    ...
    def test_multi_stage_workflow(self, llm_config: Any) -> None:
        ...
        workflow = Workflow("multi_stage")

        # Stage 1: Initial analysis
        stage1 = Node.agent("stage1", "Initial analysis", "stage1_001")
        check1 = Node.condition("check1", "stage1_complete == true")

        # Stage 2: Processing
        stage2 = Node.agent("stage2", "Secondary processing", "stage2_001")

        # Stage 3: Final
        stage3 = Node.agent("stage3", "Final processing", "stage3_001")
